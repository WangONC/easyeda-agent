...
for(const mode of ['normal','save-reject','close-retained','open-wrong','late-close','target-mismatch','last-tab-tree-null'] as const)test('checkpoint controlled reload '+mode,async()=>{
 ...
 const r:Request={protocol:V2,action:'document.open',action_revision:'1',schema:'x',request_id:'r',operation_id:'o',target_ref:target,input:{uuid:'d',reload:true},budget_ms:mode==='late-close'?10:1000};
 ...
 const h=await pending;assert.equal(h.verification.verdict==='satisfied',['normal','last-tab-tree-null'].includes(mode));assert.equal(saved,mode==='target-mismatch'?0:1);assert.equal(opened,['normal','open-wrong','last-tab-tree-null'].includes(mode)?1:0);assert.ok(closed<=1);
