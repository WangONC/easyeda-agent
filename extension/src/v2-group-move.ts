...
export function groupMove(plan: (p: Record<string, unknown>) => Promise<Plan>, serialize: (c: Comp) => Record<string, unknown>, normalize: (x: unknown) => number[]): NativeAction {
    return { mode: 'V2_NATIVE', scope: 'DESIGN_CONTENT', validate: declaredReadFields('schematic.group.move'), run: async (c) => {
            ...
            const wireState = (w: Wire) => ({ line: normalize(w.getState_Line()), net: w.getState_Net(), color: w.getState_Color(), lineWidth: w.getState_LineWidth(), lineType: w.getState_LineType() });
            ...
            const wp = [...beforeW].filter(([id]) => p.wantIds.has(id)).map(([id, w]) => { const state = wireState(w), shifted = state.line.map((n, i) => n + (i % 2 ? p.dy : p.dx)); const merged = shifted.length >= 8 && shifted.length % 4 === 0; const lines = merged ? Array.from({ length: shifted.length / 4 }, (_, i) => shifted.slice(i * 4, i * 4 + 4)).filter(a => Math.abs(a[0] - a[2]) > 1e-6 || Math.abs(a[1] - a[3]) > 1e-6) : [shifted]; if (!lines.length || lines.some(a => a.length < 4 || a.length % 2 || a.some(n => !Number.isFinite(n))))
                throw Error('V2_WIRE_SHAPE'); return { id, state, lines, merged, ids: [] as string[], attempts: 0 }; });
            ...
            const segments = (line: number[]) => { const out: string[] = []; if (line.length >= 8 && line.length % 4 === 0) {
                for (let i = 0; i < line.length; i += 4)
                    out.push(canonical([line.slice(i, i + 2), line.slice(i + 2, i + 4)].sort()));
            }
            else {
            ...
            c.prepare(async () => {
                ...
                if (probeAttempt && !probeId || [...flagAttempts].some(id => !flagIds.has(id)) || wp.some(w => w.attempts > w.ids.length) || [...newFlags].some(id => before.has(id)) || [...newWires].some(id => beforeW.has(id)) || [...cm.keys()].some(id => !before.has(id) && !newFlags.has(id) && id !== probeId) || [...wm.keys()].some(id => !beforeW.has(id) && !newWires.has(id)) || [...before].some(([id, x]) => !selected.has(id) && (!cm.has(id) || canonical(serialize(cm.get(id)!)) !== canonical(componentBefore.get(id)))) || [...wireBefore].some(([id, s]) => !selected.has(id) && (!wm.has(id) || canonical(wireState(wm.get(id)!)) !== s)))
                ...
                // Native may merge returned wire IDs. Compare the complete owned segment multiset,
                // including style/net, without guessing ownership from geometric proximity.
                const expected = wp.flatMap(w => w.lines.flatMap(line => segments(line).map(s => canonical([s, w.state.net, w.state.color, w.state.lineWidth, w.state.lineType])))).sort();
                const actual = [...newWires].flatMap(id => { const w = wm.get(id); return w ? segments(normalize(w.getState_Line())).map(s => canonical([s, w.getState_Net(), w.getState_Color(), w.getState_LineWidth(), w.getState_LineType()])) : []; }).sort();
                const allWireOK = wp.every(w => !wm.has(w.id) && w.ids.length === w.lines.length) && canonical(actual) === canonical(expected);
