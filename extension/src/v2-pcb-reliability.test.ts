...
test('add-components batch verifies explicit EP pin to numeric footprint pad relation',async()=>{
 let created=false,writes=0;const nets=new Map(['1','2','3','4'].map(n=>[n,'']));
 const comp={getState_PrimitiveId:()=> 'usb',getState_Component:()=>({uuid:'generic-device',libraryUuid:'generic-library'}),getState_Footprint:()=>({uuid:'generic-footprint',libraryUuid:'generic-library'}),getState_X:()=>10,getState_Y:()=>20,getState_Rotation:()=>0,getState_Layer:()=>1,getState_Designator:()=> 'J1',getState_UniqueId:()=> 'usb-unique',getState_OtherProperty:()=>({})};
 const pads=()=>[...nets].map(([number])=>({getState_PrimitiveId:()=> 'pad-'+number,getState_PadNumber:()=>number,getState_Net:()=>nets.get(number)}));
 (globalThis as any).eda={lib_Device:{get:async()=>({uuid:'generic-device',libraryUuid:'generic-library',association:{footprint:{uuid:'generic-footprint',libraryUuid:'generic-library'}}})},pcb_PrimitiveComponent:{getAll:async()=>created?[comp]:[],create:async()=>{writes++;created=true;return comp},modify:async()=>{writes++;return comp},getAllPinsByPrimitiveId:async()=>pads()},pcb_PrimitivePad:{modify:async(id:string,p:any)=>{writes++;nets.set(id.slice(4),p.net)}},pcb_Document:{startCalculatingRatline:async()=>{writes++;return true}}};
 const item={libraryUuid:'generic-library',uuid:'generic-device',designator:'J1',uniqueId:'usb-unique',nets:{'1':'SHELL','2':'SHELL','3':'SHELL','4':'SHELL'},pin_nets:{EP1:'SHELL',EP2:'SHELL',EP3:'SHELL',EP4:'SHELL'},pin_to_pad_map:{EP1:'1',EP2:'2',EP3:'3',EP4:'4'},x:10,y:20,layer:1,rotation:0};
 const result=await new ControlledExecutor(()=>addPcbComponentsBatch(),async()=>pcbTarget).execute(request('pcb.add_components_batch','mapped',{client_transaction_id:'mapped',components:[item]}),'mapped-digest');
 assert.equal(result.verification.verdict,'satisfied');assert.equal((result.value as any).readback_verified,true);assert.deepEqual((result.value as any).pin_pad_relations.map((x:any)=>[x.pin,x.pad,x.verified]),[['EP1','1',true],['EP2','2',true],['EP3','3',true],['EP4','4',true]]);
 const before=writes,bad={...item,designator:'J2',uniqueId:'bad',pin_to_pad_map:{...item.pin_to_pad_map,EP4:'9'}};
 const rejected=await new ControlledExecutor(()=>addPcbComponentsBatch(),async()=>pcbTarget).execute(request('pcb.add_components_batch','bad-map',{client_transaction_id:'bad-map',components:[bad]}),'bad-digest');
 assert.equal(rejected.effects.effect_started,false);assert.equal(writes,before);
});
