...
for (const mode of ['normal', 'late-create', 'late-delete', 'wrong-patch', 'drift', 'wire-reorder', 'wire-corrupt'] as const)
    test('replacement staged/final identity ' + mode, async () => {
        ...
        if (mode === 'normal' || mode === 'wire-reorder')
