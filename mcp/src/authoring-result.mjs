...
export function authoringResult(action, invocation, payload = {}) {
 ...
 const execution = interpret({action,payload,operationId:evidence.execution?.operation_id,parentOperationId:evidence.execution?.parent_operation_id,expectedTarget:evidence.execution?.expected_target,id:typeof evidence.execution?.request_id === 'string' ? evidence.execution.request_id : evidence.id}, evidence);
