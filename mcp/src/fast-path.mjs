...
export const FAST_ACTIONS = {
 ...
 easyeda_placement_preflight: 'placement.preflight',
 easyeda_placement_apply_batch: 'placement.apply_batch',
};
...
const placement={type:'object',properties:{primitiveId:string,x:number,y:number,rotation:number,layer:{type:'integer',enum:[1,2]},locked:{type:'boolean'}},required:['primitiveId','x','y','rotation','layer'],additionalProperties:false};
const props = {
 ...
 'route.apply_batch': { base_revision:string, plan_hash:string, operations:{type:'array',items:operation,minItems:1,maxItems:512} },
 'placement.preflight': {base_revision:string,placements:{type:'array',items:placement,minItems:1,maxItems:256}},
 'placement.apply_batch': {base_revision:string,plan_hash:string,placements:{type:'array',items:placement,minItems:1,maxItems:256}},
};
export function fastTools() {
 return Object.entries(FAST_ACTIONS).map(([name, action]) => ({ name,
  ...
  inputSchema:{type:'object',properties:{project:string,doc:{...string,description:'Active PCB document UUID (not name). No navigation occurs.'},window:string,...props[action]}, required:['project','doc',...(action==='route.preflight'?['base_revision']:action==='route.apply_batch'?['base_revision','plan_hash','operations']:action==='placement.preflight'?['base_revision','placements']:action==='placement.apply_batch'?['base_revision','plan_hash','placements']:[])],additionalProperties:false},
  annotations:{readOnlyHint:!action.endsWith('apply_batch'),destructiveHint:action.endsWith('apply_batch'),idempotentHint:false,openWorldHint:false},
 }));
}
