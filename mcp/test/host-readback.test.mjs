...
test('real Host MCP to CLI to daemon to Connector read and direct receipt projection',{skip:!enabled},async()=>{
 assert.ok(process.env.EASYEDA_HOST_WINDOW,'explicit logical window required');
 const window=process.env.EASYEDA_HOST_WINDOW;
 ...
 try{
  ...
  const request={action:spec.name,input:{},window};
  if(process.env.EASYEDA_HOST_EVIDENCE_FILE)fs.writeFileSync(process.env.EASYEDA_HOST_EVIDENCE_FILE,JSON.stringify({window,request,stage:'dispatch'},null,2)+String.fromCharCode(10));
  ...
  assert.ok(result.operation_id);const op=result.operation_id;assert.equal(result.outcome,'SUCCEEDED');assert.equal(result.effects.effect_started,false);
  ...
  if(process.env.EASYEDA_HOST_EVIDENCE_FILE)fs.writeFileSync(process.env.EASYEDA_HOST_EVIDENCE_FILE,JSON.stringify({window,result,stored:stored.structuredContent},null,2)+'\n');
