...
const allActions=process.argv.includes('--all');
const rows=[];for(const row of inventory.actions){if(!allActions && migratedBefore.has(row.action))continue;const handler=mapping.get(row.action),body=declarations.get(handler),keys=new Set();
...
assert.equal(rows.length,allActions?151:70);assert.ok(rows.every(r=>r.missing_direct_parameters.length===0),'baseline direct parameter omitted');
if(process.argv.includes('--write'))fs.writeFileSync(allActions?'docs/execution-v2/round3-business-audit.json':'docs/execution-v2/round2-business-audit.json',JSON.stringify({baseline:inventory.baseline,round1:'f96960c0487ca0aff8c5095220f496a22e5d1398',scope:allActions?'All 151 actions; direct handler parameters only, not a semantic equivalence proof':'70 actions not native at Round 1; direct handler parameters only, not a transitive proof',actions:rows},null,2)+'\n');
