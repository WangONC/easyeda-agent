...
let input = fs.readFileSync('extension/src/execution.ts','utf8').replace("import contracts from './action-contracts.json';",'const contracts = '+fs.readFileSync('extension/src/action-contracts.json','utf8')+';');
input = input.replace("import evidenceInventory from '../../internal/protocol/execution-evidence.json';", () => 'const evidenceInventory = '+fs.readFileSync('internal/protocol/execution-evidence.json','utf8')+';');
