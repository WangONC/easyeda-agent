...
`RETIRED_UNRESOLVED` scope quarantine。runtime 先用 retirement 后新建的 exact-target NONE-effect read receipts
自动 requalify；若结果为 `AUTO_REQUALIFIED`，受影响 scope 已正式解除。只有自动读取失败且 health 仍列出
`required_requalification` 时，才手工做 exact rebind、fresh snapshot/checkpoint，并用正式
`operation requalify` 消除隔离。不要复用 retirement 前 receipt，也不要通过强制读、另起脚本、重启或 replay 绕过 ownership。
